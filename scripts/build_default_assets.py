...
def process_sr_models(wakenet_model_dirs, multinet_model_dirs, build_dir, assets_dir, nsnet_model_dirs=None):
    ...
    # Copy nsnet (noise suppression) models if available
    if nsnet_model_dirs:
...
def get_nsnet_model_paths(esp_sr_model_path):
    """
    Get the full paths to the nsnet (noise suppression) model directories
    Returns a list of valid model paths (nsnet1, nsnet2, etc.)
    """
    nsnet_dir = os.path.join(esp_sr_model_path, 'nsnet_model')
    if not os.path.exists(nsnet_dir):
        print(f"Warning: nsnet model directory not found: {nsnet_dir}")
        return []
    
    valid_paths = []
    # Check for nsnet1, nsnet2
...
def build_assets_integrated(wakenet_model_paths, multinet_model_paths, text_font_path, emoji_collection_path, extra_files_path, output_path, multinet_model_info=None, nsnet_model_paths=None):
    ...
    try:
        ...
        # Process each component (include nsnet models)
        srmodels = process_sr_models(wakenet_model_paths, multinet_model_paths, temp_build_dir, assets_dir, nsnet_model_paths) if (wakenet_model_paths or multinet_model_paths or nsnet_model_paths) else None
...
def main():
    ...
    # Get nsnet (noise suppression) model paths
    nsnet_model_paths = get_nsnet_model_paths(args.esp_sr_model_path)
    if nsnet_model_paths:
        print(f"  nsnet models: {len(nsnet_model_paths)} model(s) found (will be packaged for noise reduction)")
