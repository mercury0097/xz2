...
static const std::unordered_map<std::string, const lv_image_dsc_t *>
    emoji_to_icon = {
        ...
        {"🦁", &lion_face},    {"🐧", &penguin},      {"🦜", &parrot},
        {"🐉", &dragon},       {"🦄", &unicorn_face}, {"🐘", &elephant},
        {"🦒", &giraffe_face}, {"🦏", &rhinoceros},   {"🦥", &sloth},
        {"🐺", &wolf}};
...
void LcdDisplay::InitializeLcdThemes() {
  ...
  light_theme->set_background_color(lv_color_hex(
      0x000000)); // rgb(0, 0, 0) 纯黑色背景（适配 Otto 机器人眼睛）
  light_theme->set_text_color(
      lv_color_hex(0x2C3E50)); // rgb(44, 62, 80) 深蓝灰色文字
  light_theme->set_chat_background_color(
      lv_color_hex(0xE8F5E9)); // rgb(232, 245, 233) 淡绿色对话框背景
  light_theme->set_user_bubble_color(
      lv_color_hex(0x4CAF50)); // rgb(76, 175, 80) 鲜绿色用户气泡
  light_theme->set_assistant_bubble_color(
      lv_color_hex(0x64B5F6)); // rgb(100, 181, 246) 蓝色助手气泡
  light_theme->set_system_bubble_color(
      lv_color_hex(0xFFEB3B)); // rgb(255, 235, 59) 黄色系统提示
  light_theme->set_system_text_color(
      lv_color_hex(0x5D4037)); // rgb(93, 64, 55) 深褐色系统文字
  light_theme->set_border_color(
      lv_color_hex(0x03A9F4)); // rgb(3, 169, 244) 蓝色边框
  light_theme->set_low_battery_color(
      lv_color_hex(0xFF5722)); // rgb(255, 87, 34) 橙红色低电量
  ...
  dark_theme->set_text_color(
      lv_color_hex(0xE0E0E0)); // rgb(224, 224, 224) 亮灰色文字
  dark_theme->set_chat_background_color(
      lv_color_hex(0x283593)); // rgb(40, 53, 147) 蓝紫色对话框
  dark_theme->set_user_bubble_color(
      lv_color_hex(0x00E676)); // rgb(0, 230, 118) 荧光绿用户气泡
  dark_theme->set_assistant_bubble_color(
      lv_color_hex(0x448AFF)); // rgb(68, 138, 255) 亮蓝色助手气泡
  dark_theme->set_system_bubble_color(
      lv_color_hex(0xFF6E40)); // rgb(255, 110, 64) 橙色系统提示
  dark_theme->set_system_text_color(
      lv_color_hex(0xFFFFFF)); // rgb(255, 255, 255) 白色系统文字
  dark_theme->set_border_color(
      lv_color_hex(0x00E5FF)); // rgb(0, 229, 255) 青色边框
  dark_theme->set_low_battery_color(
      lv_color_hex(0xFF1744)); // rgb(255, 23, 68) 鲜红色低电量
...
void LcdDisplay::SetPetEmoji(const char *emoji) {
  ...
  if (it != emoji_to_icon.end()) {
    ...
    ESP_LOGI(TAG, "✅ Set pet icon for emoji: %s", emoji);
  } else {
    // 未找到对应图标，使用默认猫咪图标
    lv_img_set_src(pet_emoji_img_, &cat);
    lv_obj_clear_flag(pet_emoji_img_, LV_OBJ_FLAG_HIDDEN);
    ESP_LOGW(TAG,
             "❌ No icon found for emoji: '%s' (len=%d), using default cat",
