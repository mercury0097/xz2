    ...
    if (!audio_decode_queue_.empty() &&
      ...
      if (opus_decoder_->Decode(std::move(packet->payload), task->pcm)) {
        ...
        if (opus_decoder_->sample_rate() != codec_->output_sample_rate()) {
          ...
          if (resample_log_count < 3) { // 只打印前3次，避免刷屏
            ESP_LOGW(TAG, "🔄 重采样: %d Hz → %d Hz (可能影响音质)",
...
void AudioService::EnableWakeWordDetection(bool enable) {
  // 如果 wake_word_ 为空且 models_list_ 也为空，尝试从 model 分区加载模型
  if (!wake_word_ && !models_list_) {
    ESP_LOGI(TAG, "尝试从 model 分区加载唤醒词模型...");
    models_list_ = esp_srmodel_init("model");
    if (models_list_) {
      ESP_LOGI(TAG, "成功从 model 分区加载模型");
      // 创建唤醒词检测器
#if CONFIG_IDF_TARGET_ESP32S3 || CONFIG_IDF_TARGET_ESP32P4
...
void AudioService::SetBargeInContextMode(bool in_conversation) {
  // Barge-in 功能已禁用，此函数保留但不执行任何操作
  ESP_LOGD(TAG, "Barge-in 功能已禁用");
}
