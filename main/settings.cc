    ...
    if (read_write_) {
        ...
        if (err != ESP_OK) {
            if (err == ESP_ERR_NVS_NOT_ENOUGH_SPACE) {
                ESP_LOGE(TAG, "NVS out of space! Cannot save %s=%s in namespace %s", 
                         key.c_str(), value.c_str(), ns_.c_str());
            } else {
                ESP_LOGE(TAG, "Failed to set %s in namespace %s: %s", 
                         key.c_str(), ns_.c_str(), esp_err_to_name(err));
            }
            return;
        }
        dirty_ = true;
...
void Settings::SetInt(const std::string& key, int32_t value) {
    if (read_write_) {
        ...
        if (err != ESP_OK) {
            if (err == ESP_ERR_NVS_NOT_ENOUGH_SPACE) {
                ESP_LOGE(TAG, "NVS out of space! Cannot save %s=%d in namespace %s", 
                         key.c_str(), value, ns_.c_str());
            } else {
                ESP_LOGE(TAG, "Failed to set %s in namespace %s: %s", 
                         key.c_str(), ns_.c_str(), esp_err_to_name(err));
            }
...
void Settings::SetInt64(const std::string& key, int64_t value) {
    if (read_write_) {
        esp_err_t err = nvs_set_i64(nvs_handle_, key.c_str(), value);
        if (err != ESP_OK) {
            if (err == ESP_ERR_NVS_NOT_ENOUGH_SPACE) {
                ESP_LOGE(TAG, "NVS out of space! Cannot save %s=%lld in namespace %s", 
                         key.c_str(), value, ns_.c_str());
            } else {
                ESP_LOGE(TAG, "Failed to set %s in namespace %s: %s", 
                         key.c_str(), ns_.c_str(), esp_err_to_name(err));
