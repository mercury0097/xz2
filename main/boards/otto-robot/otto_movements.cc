...
void Otto::OscillateServos(int amplitude[SERVO_COUNT], int offset[SERVO_COUNT],
...
void Otto::Execute(int amplitude[SERVO_COUNT], int offset[SERVO_COUNT],
                   int period, double phase_diff[SERVO_COUNT],
                   float steps = 1.0) {
  if (GetRestState() == true) {
    SetRestState(false);
...
//---------------------------------------------------------
//-- Otto movement: Look around (left or right)
//--  Parameters:
//--    period: 动作周期(ms)
...
//---------------------------------------------------------
//-- Otto movement: swinging side to side without touching the floor with the
// heel
//--  Parameters:
//--     steps: Number of steps
//--     T : Period
...
void Otto::Moonwalker(float steps, int period, int height, int dir) {
  ...
  //-- The two Otto's feet are equivalent to a minimal configuration. It is
  // known
...
//--    ease_type: 缓动类型
//---------------------------------------------------------
void Otto::MoveServosWithEase(int time, int servo_target[], EaseType ease_type) {
  if (GetRestState() == true) {
    SetRestState(false);
  }
...
//--    count: 路径点数量
//---------------------------------------------------------
void Otto::MoveServoPath(int servo_index, BezierWaypoint waypoints[], int count) {
...
//--  比原版更自然，有回弹效果
//---------------------------------------------------------
void Otto::HandWaveSmooth(int period, int dir) {
  if (!has_hands_) {
    return;
  }
