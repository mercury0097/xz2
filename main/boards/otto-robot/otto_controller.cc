...
private:
    ...
    while (true) {
      if (xQueueReceive(controller->action_queue_, &params,
        ...
        case ACTION_MOONWALK:
          controller->otto_.Moonwalker(params.steps, params.speed,
        ...
        case ACTION_LOOK_AROUND:
          controller->otto_.LookAround(params.speed, params.direction);
          break;
        }
  ...
  void StartActionTaskIfNeeded() {
    if (action_task_handle_ == nullptr) {
      xTaskCreate(ActionTask, "otto_action", 1024 * 3, this,
  ...
  void StartIdleActionTask() {
    if (idle_task_handle_ == nullptr) {
      xTaskCreate(IdleActionTask, "otto_idle", 1024 * 2, this,
...
public:
  ...
  void RegisterMcpTools() {
    ...
    mcp_server.AddTool(
        "self.otto.set_trim",
        "校准单个舵机位置。设置指定舵机的微调参数以调整Otto的初始站立姿态，设置"
        "将永久保存。"
        "servo_type: "
    ...
    mcp_server.AddTool(
        "self.otto.get_trims", "获取当前的舵机微调设置", PropertyList(),
        [this](const PropertyList &properties) -> ReturnValue {
          Settings settings("otto_trims", false);
...
void OttoSwing(int steps, int speed, int amount) {
  if (g_otto_controller != nullptr) {
    // ACTION_SWING = 4
    g_otto_controller->QueueAction(4, steps, speed, 0, amount);
  }
}
...
void OttoJump(int steps, int speed) {
  if (g_otto_controller != nullptr) {
    // ACTION_JUMP = 3
    g_otto_controller->QueueAction(3, steps, speed, 0, 0);
  }
}
